...
num *preenche_vetor2(int tamseed2, int tam2)
{
    ...
    for (int i = 1; i < tam2; i++) // Corrigido para come�ar a partir de 1
    {
...
{
    int i, j;
    int trocas; // Vari�vel para rastrear se houve trocas

    for (i = 0; i < tamanho - 1; i++)
    {
        trocas = 0; // Inicializa o n�mero de trocas para cada passagem
        ...
        if (trocas == 0)
            break; // Se n�o houve trocas, o vetor j� est� ordenado
    }
}
...
num *determinatipo(int tipo, int tamseed, int tamvet, int tam[])
{
    ...
    default:
        printf("\n Tipo invalido!");
        vet = NULL; // Tratar caso inv�lido retornando NULL
        break;
    }
